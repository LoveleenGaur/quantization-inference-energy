...
CONFIG = {
    # Two sizes, so the result is not an artifact of one small model.
    "models": ["Qwen/Qwen2.5-0.5B-Instruct", "Qwen/Qwen2.5-1.5B-Instruct"],
    "conditions": ["fp16", "nf4"],
    "n_prompts": 8,
    "max_new_tokens": 64,
    "reps_per_block": 4,        # 2 blocks x 4 reps = 8 runs per condition per model
...
print("models:", ", ".join(CONFIG["models"]))
_total = (len(CONFIG["models"])*CONFIG["blocks"]*len(CONFIG["conditions"])*CONFIG["reps_per_block"])
print(f"measured runs: {_total} | model loads: "
      f"{len(CONFIG['models'])*CONFIG['blocks']*len(CONFIG['conditions'])}")
print("estimated 30 to 40 min on a T4; every run is saved as it completes")
print(f"output -> {OUT}")
...
COLS = ["model","condition","block","rep","warmup","new_tokens","seconds","tokens_per_s",
        ...
        "mem_baseline_mb","peak_mem_mb","model_mem_mb",
        "perplexity","max_temp_c","idle_temp_c","throttle","ok","error"]
...
def release(*objs):
    """Free VRAM and host RAM between conditions, then reset the peak counter.

    The first lite run showed peak memory rising by ~187 MB in block 1 for both
    conditions, because allocations from the previous condition were still
    resident when the peak counter was read. Flushing twice and resetting the
    counter after the flush removes that residue, and we also record the
    baseline so model memory can be reported net of anything left over.
    """
    for o in objs:
        try: del o
...
if RAW.exists():
    ...
    for r in pd.read_csv(RAW).itertuples():
        done.add((r.model, r.condition, int(r.block), int(r.rep)))
    print(f"resuming, {len(done)} runs already recorded")

total = (len(CONFIG["models"]) * CONFIG["blocks"]
         * len(CONFIG["conditions"]) * CONFIG["reps_per_block"])
...
for model_id in CONFIG["models"]:
  ...
  for block in range(CONFIG["blocks"]):
    ...
    for cond in order:
        todo = [r for r in range(CONFIG["reps_per_block"])
                if (model_id, cond, block, r) not in done]
        if not todo:
            print(f"{short} | block {block} | {cond}: all reps recorded, skipping"); continue

        model = tok = None
        try:
            base_mb = release()                     # flush, reset peak, read baseline
            idle_w, idle_t = idle_baseline()
            model, tok = load(model_id, cond)       # ONE load serves every rep here
            ...
            for rep in todo:
                ...
                rec = {"model": model_id, "condition": cond, "block": block, "rep": rep,
                       ...
                       "mem_baseline_mb": round(base_mb,1),
                       "peak_mem_mb": round(peak,1),
                       "model_mem_mb": round(peak - base_mb,1),
                       "perplexity": round(q,3),
                ...
                eta = (time.perf_counter()-t_start)/max(n_done-len(done),1)*(total-n_done)
                print(f"{short[:18]:18s} b{block} {cond:5s} r{rep}  "
                      f"{rec['wh_per_1k_tokens']:7.4f} Wh/1k  "
                      f"{rec['tokens_per_s']:6.1f} tok/s  {rec['model_mem_mb']:6.0f} MB  "
                      f"ppl {q:6.2f}{flag}   [{n_done}/{total}] eta {eta/60:.0f}m")
        except Exception as ex:
            print(f"{short} b{block} {cond:5s} FAILED: {type(ex).__name__}: {ex}")
            save_row({"model": model_id, "condition": cond, "block": block,
...
if good.empty:
    ...
    for mid, g in good.groupby("model"):
        a = g[g.condition=="fp16"]; b = g[g.condition=="nf4"]
        if not (len(a) and len(b)): continue
        de = (b.wh_per_1k_tokens.mean()-a.wh_per_1k_tokens.mean())/a.wh_per_1k_tokens.mean()*100
        dm = (b.model_mem_mb.mean()-a.model_mem_mb.mean())/a.model_mem_mb.mean()*100
        ds = (b.tokens_per_s.mean()-a.tokens_per_s.mean())/a.tokens_per_s.mean()*100
        dp = (b.perplexity.mean()-a.perplexity.mean())/a.perplexity.mean()*100
        print(f"\n{mid}  (n = {len(a)} fp16, {len(b)} nf4)")
        print(f"  energy per 1k tokens {de:+7.1f}%   (negative = a saving)")
        print(f"  model memory         {dm:+7.1f}%")
        print(f"  throughput           {ds:+7.1f}%")
        print(f"  perplexity           {dp:+7.1f}%   (positive = worse output)")
        if len(a)>1 and len(b)>1:
            tt,p = stats.ttest_ind(b.wh_per_1k_tokens, a.wh_per_1k_tokens, equal_var=False)
            pooled = ((a.wh_per_1k_tokens.var(ddof=1)+b.wh_per_1k_tokens.var(ddof=1))/2)**0.5
            dcoh = (b.wh_per_1k_tokens.mean()-a.wh_per_1k_tokens.mean())/pooled
            overlap = a.wh_per_1k_tokens.max() >= b.wh_per_1k_tokens.min()
            print(f"  Welch p = {p:.5f}, Cohen's d = {dcoh:.2f}, ranges overlap: {overlap}")
        print("  per block (order reversed in block 1):")
        for blk, gb in g.groupby("block"):
            f_ = gb[gb.condition=="fp16"].wh_per_1k_tokens.mean()
            q_ = gb[gb.condition=="nf4"].wh_per_1k_tokens.mean()
            if f_ == f_ and q_ == q_:
                print(f"    block {blk}: {(q_-f_)/f_*100:+.1f}%")
